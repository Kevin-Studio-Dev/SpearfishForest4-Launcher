...
const dataPath = path.join(sysRoot, '.spearforest')
...
exports.initializeMinecraftOptions = function(serverid){
    try {
        // resourcepacks 디렉토리만 생성
        const resourcepacksDir = exports.getResourcePacksPath(serverid)
        fs.ensureDirSync(resourcepacksDir)
    } catch (err) {
        logger.error(`Failed to create resourcepacks directory for server ${serverid}:`, err)
    }
}